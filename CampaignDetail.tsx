...
export function CampaignDetail({ campaign, onBack, onUpdateCampaign }: CampaignDetailProps) {
  ...
  const statusConfig: Record<Contact["status"], { label: string; cls: string; bg: string; icon: React.ReactNode }> = {
    ...
    sent:    { label: "Sent",    cls: "text-indigo-400", bg: "bg-indigo-500/10 border-indigo-500/20", icon: <Mail className="w-3 h-3" /> },
    positive:{ label: "Happy",   cls: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20", icon: <ThumbsUp className="w-3 h-3" /> },
    negative:{ label: "Unhappy", cls: "text-rose-400", bg: "bg-rose-500/10 border-rose-500/20", icon: <ThumbsDown className="w-3 h-3" /> },
    unopened:{ label: "Unopened",cls: "text-amber-400", bg: "bg-amber-500/10 border-amber-500/20", icon: <AlertCircle className="w-3 h-3" /> },
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center gap-4 mb-8 flex-wrap">
          ...
          <a href={campaign.googleMapsUrl} target="_blank" rel="noopener noreferrer"
            ...
            <MapPin className="w-4 h-4 text-rose-400" /> Google Maps <ExternalLink className="w-3.5 h-3.5" />
          </a>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: "Total Contacts", value: contacts.length, icon: Shield, color: "violet" },
            { label: "Responded", value: sent, icon: Mail, color: "indigo" },
            { label: "→ Google Maps", value: positive.length, icon: ThumbsUp, color: "emerald" },
            { label: "Private Feedback", value: negative.length, icon: ThumbsDown, color: "rose" },
          ].map((s) => {
            const colors: Record<string, { border: string; icon: string; num: string }> = {
              violet:  { border: "border-violet-500/20",  icon: "text-violet-400 bg-violet-500/10 border-violet-500/20",  num: "text-white" },
              indigo:  { border: "border-indigo-500/20",  icon: "text-indigo-400 bg-indigo-500/10 border-indigo-500/20",  num: "text-violet-300" },
              emerald: { border: "border-emerald-500/20", icon: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20", num: "text-emerald-400" },
              rose:    { border: "border-rose-500/20",    icon: "text-rose-400 bg-rose-500/10 border-rose-500/20",    num: "text-rose-400" },
            };
        ...
        <div className="bg-white/[0.03] backdrop-blur-md border border-white/[0.06] rounded-2xl p-6 mb-8">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xl font-black ${pct >= 70 ? "text-emerald-400" : pct >= 40 ? "text-amber-400" : "text-rose-400"}`}>
                {pct}%
              </span>
            </div>
          ...
          <div className="h-3 bg-black/40 rounded-full overflow-hidden flex border border-white/[0.04]">
            <div className="bg-emerald-500 h-full transition-all duration-700 shadow-[0_0_10px_rgba(52,211,153,0.4)]"
              style={{ width: `${contacts.length > 0 ? (positive.length / contacts.length) * 100 : 0}%` }} />
            <div className="bg-rose-500 h-full transition-all duration-700 shadow-[0_0_10px_rgba(244,63,94,0.4)]"
              style={{ width: `${contacts.length > 0 ? (negative.length / contacts.length) * 100 : 0}%` }} />
          </div>
          <div className="flex gap-5 mt-3">
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_6px_rgba(52,211,153,0.5)]" /><span className="text-zinc-500 text-[11px]">Happy ({positive.length})</span></div>
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-rose-500 shadow-[0_0_6px_rgba(244,63,94,0.5)]" /><span className="text-zinc-500 text-[11px]">Unhappy ({negative.length})</span></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  return (
                    <button key={c.id}
                      ...
                      className={`w-full flex items-center justify-between px-5 py-3.5 hover:bg-white/[0.04] transition-colors text-left ${
                        selectedContact?.id === c.id ? "bg-violet-500/[0.06] border-l-2 border-l-violet-500" : ""
                      }`}>
                      <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center font-black text-sm flex-shrink-0 ${
                          c.status === "positive" ? "bg-emerald-500/20 text-emerald-400" :
                          c.status === "negative" ? "bg-rose-500/20 text-rose-400" :
          ...
          <div className="space-y-4">
              ...
              <div className="bg-white/[0.03] backdrop-blur-md border border-white/[0.06] rounded-2xl p-5">
                <div className="flex items-center gap-3 mb-5">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg ${
                    selectedContact.status === "positive" ? "bg-emerald-500/20 text-emerald-400" :
                    selectedContact.status === "negative" ? "bg-rose-500/20 text-rose-400" :
                    "bg-white/[0.06] text-zinc-400"
                  }`}>
                ...
                {selectedContact.status === "negative" && (
                  <div className="space-y-3">
                    <div className="bg-rose-500/[0.07] border border-rose-500/20 rounded-xl p-4">
                    ...
                    <div className="bg-amber-500/[0.07] border border-amber-500/20 rounded-xl p-3 text-xs text-amber-400/80 leading-relaxed">
            ...
            {negative.length > 0 && (
              <div className="bg-white/[0.03] backdrop-blur-md border border-rose-500/20 rounded-2xl p-5">
                ...
                <div className="space-y-2 max-h-52 overflow-y-auto">
                  {negative.map((c) => (
                    <div key={c.id} className="bg-rose-500/[0.06] border border-rose-500/15 rounded-xl p-3">
                      <div className="font-bold text-rose-300 text-xs mb-1">{c.name}</div>
            ...
            {/* Google Maps card */}
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-600/80 to-indigo-700/80 border border-violet-500/30 p-5 shadow-[0_0_40px_rgba(139,92,246,0.2)] backdrop-blur-md">
              ...
              <div className="relative z-10">
                ...
                <a href={campaign.googleMapsUrl} target="_blank" rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full bg-white text-violet-700 rounded-xl py-3 text-sm font-black hover:bg-violet-50 transition-colors shadow-lg">
